...
url = "https://daymet.ornl.gov/single-pixel/api/data?lat=34.9455" \
       "&lon=-113.2549&vars=prcp&start=1989-01-01&end=2020-12-31" \
       "&format=csv"
...
oct_data = flow_data[flow_data['month'] == 10]
...
ax.plot(nov_mean.index, nov_mean.values, color='steelblue',
        label='November mean')
...
fig, ax = plt.subplots()
ax.plot(oct_median.index, oct_median.values, color='crimson',
        label='October median')
ax.plot(nov_median.index, nov_median.values, color='steelblue',
...
oct_psr = psr_daily[psr_daily['month'] == 10]
psr_mean = oct_psr.groupby('day')['Precipitation (mm)'].mean()
...
psr_bool = (oct_psr['Precipitation (mm)'] > 0).astype(int)
fgz_bool = (oct_fgz['Precipitation (mm)'] > 0).astype(int)
cvd_bool = (oct_cvd['prcp (mm/day)'] > 0).astype(int)
...
# New for week 9 - add climatological component - based on 12% chance of precip
# per day in month of October, 12%*7=84%, 1 day of precip expected per week
...
def flowcast(obs, mean, med, mean2, med2, a, b, day):
    ...
    for i in range(7):
        ...
        week2[i] = mean2.values[i]*0.4+med2.values[i]*0.4+obs*0.2
...
# Climatological index for October of 1 day per week
# Cumulative precip value of 100 this week, 100/50=2
# Day should be set to 24 for week 9.

last_flow = recent_flow["flow"].iloc[-1]
precip = 2
climo = 1
date = 24
flowcast1, flowcast2 = flowcast(last_flow, oct_mean, oct_median,
                                nov_mean, nov_median,
                                precip, climo, date)
