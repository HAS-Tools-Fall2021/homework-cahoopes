...
# %%
# Net CDF data
# Download precipitation data for region of Verde Watershed above streamgauge
# Based on previous week's map, data ranges are 34.4-35.9N, 113.2-111.1W
# CPC data chosen instead of NCEP example, higher spatial resolution
data_path = os.path.join('cpc_precip_verde_jan89oct21.nc')

# Read in the dataset as an x-array and view it
dataset = xr.open_dataset(data_path)
dataset

# 3 lat values, 4 lon values, 11992 time values
dataset['precip']['lat'].size
dataset['precip']['lon'].size
dataset["precip"]["time"].size

# Extract precip values as a numpy array
precip_val = dataset["precip"].values
precip_val.shape
type(precip_val)

# %%
# Make a timeseries plot for a single point
# Try to get point as close to streamflow gauge as possible
# Stream gauge:  34.44833333 W, -111.7891667 E (~248.21 E)
lat = dataset["precip"]["lat"].values
lon = dataset["precip"]["lon"].values
print("Lon, Lat values:", lon[2], lat[2])
one_point = dataset["precip"].sel(lat=lat[0], lon=lon[0])
...
ax.set(title="Precipitation Time Series For Gauge Location")
f.savefig("nc_timeseries.png")

# %%
# Making a spatial map of the temporal mean
precip = dataset["precip"].values
mean_precip = np.nanmean(precip, 0)
# start_date = "2000-01-01"
# end_date = "2000-01-01"
# timeslice = dataset["precip"].sel(
#    time=slice(start_date, end_date))
f, ax = plt.subplots(figsize=(10, 10))
c = plt.pcolormesh(lon, lat, mean_precip, cmap='cividis')
ax.set(title='Mean Daily Precipitation, 1989-Present (mm)',
...
flow_url = "https://waterdata.usgs.gov/nwis/dv?cb_00060=on&format=rdb" \
           ...
           "&period=&begin_date=1989-01-01&end_date=2021-10-31"
flow_data = pd.read_table(flow_url, sep='\t', skiprows=30,
                          names=['agency_cd', 'site_no', 'datetime', 'flow',
                                 'code'], parse_dates=['datetime'])

# Extract flow data, convert into a series of how it changes day-to-day
# End goal of performing a linear regression, using precipitation
# at and above the gauge to predict streamflow change from last observation
# Do a forward difference so flow will lag behind precip by one day
flow_only = flow_data['flow']
diff_flow = np.zeros(len(flow_only))
for i in range(len(flow_only)):
    if i == 0:
        diff_flow[i] = 0
    else:
        diff_flow[i] = flow_only[i]-flow_only[i-1]

# Work with the Precipitation NetCDF file to get a spatial mean for each day
# One day where all data is missing, remove it, will break regression otherwise
# No other missing data, can use normal mean
precip_mean = np.mean(precip_val, axis=(1, 2))
precip_mean[np.isnan(precip_mean)] = 0

# High amount of zero-value days, may mess with regression if unadressed
# Remove all zero values beforehand
nozeros = np.nonzero(precip_mean)
precip_reg = precip_mean[nozeros]
precip_reg = precip_reg.reshape(-1, 1)
flow_reg = diff_flow[nozeros]

# %%
# Now ready to perform a linear regression on the 2 timeseries
# Start by training the model on reanalysis data
model = LinearRegression().fit(precip_reg, flow_reg)
r2 = model.score(precip_reg, flow_reg)
b = model.intercept_
m = model.coef_
print('Regression equation is y=', m, 'x+', b)
print('R^2 value is', r2)

# %%
# Now add in forecast data, predict streamflow based on regression
# Download accumulated precip at 1 week, average into individual days
data_path = os.path.join('gfs.0p25.2021110712.f186.grib2.nc')

# Read in the dataset as an x-array and look at it
dataset = xr.open_dataset(data_path)
dataset
...
# %%
# Get the mean forecast values at 1 week over the basin, use to predict
# flow according to regression
# Unlike reanalysis data, no time dimension to worry about
forecast_val = dataset["precip"].values
forecast_val.shape
type(forecast_val)
forecast_mean = np.mean(forecast_val)
forecast_mean[np.isnan(forecast_mean)] = 0

# Divide data into 7 days, apply regression for week 1 forecast
# Precip not a factor for week 2, apply old method,function,
# using first week's forecast as input, get climatology after this
last_flow = flow_data.iloc[-1]
daily_forecast = np.zeros(7)
river_forecast = np.eros(7)
for i in range(7):
    daily_forecast[i] = forecast_mean/7.0
    if i == 0:
        river_forecast[i] = last_flow+(m*daily_forecast[i]+b)
    else:
        river_forecast[i] = river_forecast[i-1]+(m*daily_forecast[i]+b)

flowcast1 = round(np.mean(river_forecast))

# %%
# Could not get linear regression to give reasonable results,
# eventually reverted to old method, with a few minor alterations
# Tried time-lagging, adding back in zeros, removing outliers,
# nothing helped
# Must read in flow data again, previous reading length-limited
# so size would match with availiable precip reanalysis
flow_url = "https://waterdata.usgs.gov/nwis/dv?cb_00060=on&format=rdb" \
           "&site_no=09506000&referred_module=sw" \
...
# Day should be set to 6 for week 11.
