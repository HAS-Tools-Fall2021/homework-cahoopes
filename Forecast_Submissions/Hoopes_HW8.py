...
fig, ax = plt.subplots()
ax.plot(recent_data['flow'], color='steelblue', label='Last Month')
...
ax.legend()
plt.show()
fig.savefig('Recent_flow_week8.png')
...
ax.set(title="Mean October Flow", xlabel="Day of Month",
       ylabel="October Mean Flow [cfs]")
ax.legend()
plt.show()
...
ax.set(title="Median October Flow", xlabel="Day of Month",
       ylabel="October Median Flow [cfs]")
ax.legend()
plt.show()
...
def flowcast(obs, mean, med, a, day):
    ...
    mean and median flow to weight the interpolation.

    For the first week, additional weighting comes from precipitation,
    ...
    a - number of days where precipitation is expected,
...
# Should be set to 17 for week 8.
